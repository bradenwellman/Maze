...
// #####################################################################################################################
//																													  ##
// 									THESE ARE JUST THE DEFINITIONS OF ALL THE FUNCTIONS								  ##
//																													  ##
// #####################################################################################################################

const string fileArray[NUM_FILES] = { "file1.txt", "file2.txt", "file3.txt", "file4.txt", "file5.txt" }; // the string array containing the file names
...
void parse_instruction(string temp, ofstream &ofs, Pathfinder* aptr);
...
string is_valid_path(vector<string> &vec, string mazeString);
...
void parse_instruction(string temp, ofstream &ofs, Pathfinder* aptr) {
	...
	if (command == "importMaze") { // command to import a new maze from a file
		if (aptr->importMaze(ss.str().substr(11, string::npos))) { // use the rest of the stringstream as importMaze input
			ofs << temp << " True" << endl;
		}
		else {
			ofs << temp << " False" << endl;
		...
		vector<string> pathVec = aptr->solveMaze(); // attempt to solve the maze;

		if (pathVec.size() == 0) { // no valid path, maze is unsolvable
			ofs << temp << "\n" << aptr->toString() << "\nPath: Unsolvable\n";
		}
		else {
			ofs << temp << "\n" << aptr->toString() << endl << is_valid_path(pathVec, aptr->toString()) << endl; //<< ":\n" << ss.str() << std::endl;
		}
	}

		// ------------------------------------------ SOLVE THE MAZE -------------------------------------------------------
	else if (command == "solveMaze") { // command to solve the maze and return a correct path if possible
		string originalMaze = aptr->toString();
		vector<string> pathVec = aptr->solveMaze(); // get the path vector returned by the student
		if(originalMaze != aptr->toString()){
			ofs << temp << "\nINVALID solution. Maze has been altered from: \n" << originalMaze
				<< "\n...to: \n\n" << aptr->toString() << endl;
		}
		else{
			ofs << temp << " " << is_valid_path(pathVec, aptr->toString()) << endl;
		}
...
string getCoords(string coordString, int &x, int &y, int &z) {
	stringstream ss;
